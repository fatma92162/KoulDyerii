...
class AdminPostController extends AbstractController
{
    public function __construct(
        ...
        private UtilisateurRepository $utilisateurRepository  // pour récupérer le bot
    ) {}
    ...
    // ✅ EXPORT CSV (version robuste sans dépendance externe)
    ...
    public function exportPosts(Request $request): Response
    {
        ...
        // Construction des données avec nettoyage
        $data = [];
        foreach ($posts as $post) {
            $title = str_replace(["\r\n", "\n", "\r"], ' ', $post->getTitle());
    ...
    // ✅ Ajouter un commentaire (avec réponse IA automatique)
    ...
    public function comment(int $id, Request $request): Response
    {
        ...
        // Enregistrement du commentaire original
        ...
        // Génération de la réponse automatique par IA
        $botUser = $this->utilisateurRepository->findOneBy(['email' => 'bot@example.com']);
        if (!$botUser) {
            // Si le bot n'existe pas, on peut utiliser l'admin courant ou créer un utilisateur factice
            // Pour simplifier, on utilise l'admin, mais il est recommandé de créer un utilisateur dédié
            $botUser = $user;
        }
        ...
        if ($aiReply) {
            ...
            $this->addFlash('success', '✅ Votre commentaire a été ajouté (réponse IA non disponible).');
        }
