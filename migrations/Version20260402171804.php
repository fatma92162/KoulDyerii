 ...
 */
final class Version20260402171804 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('DROP INDEX idClient ON commande');
        $this->addSql('DROP INDEX idProduit ON commande_produit');
        $this->addSql('DROP INDEX post_id ON commentaire');
        $this->addSql('DROP INDEX user_id ON commentaire');
        $this->addSql('DROP INDEX idx_comment_post ON commentaires');
        $this->addSql('DROP INDEX idProduit ON favoris');
        $this->addSql('DROP INDEX idVendeuse ON formation');
        $this->addSql('DROP INDEX idFormation ON inscription_formation');
        $this->addSql('DROP INDEX idCommande ON livraison');
        $this->addSql('DROP INDEX idx_user_notifications ON notification');
        $this->addSql('DROP INDEX from_user_id ON notification');
        $this->addSql('DROP INDEX post_id ON notification');
        $this->addSql('DROP INDEX commentaire_id ON notification');
        $this->addSql('ALTER TABLE password_reset_tokens CHANGE email email INT DEFAULT NULL, CHANGE date_expiration date_expiration DATETIME NOT NULL, CHANGE utilise utilise TINYINT NOT NULL');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE INDEX idClient ON commande (idClient)');
        $this->addSql('CREATE INDEX idProduit ON commande_produit (idProduit)');
        $this->addSql('CREATE INDEX post_id ON commentaire (post_id)');
        $this->addSql('CREATE INDEX user_id ON commentaire (user_id)');
        $this->addSql('CREATE INDEX idx_comment_post ON commentaires (post_id)');
        $this->addSql('CREATE INDEX idProduit ON favoris (idProduit)');
        $this->addSql('CREATE INDEX idVendeuse ON formation (idVendeuse)');
        $this->addSql('CREATE INDEX idFormation ON inscription_formation (idFormation)');
        $this->addSql('CREATE UNIQUE INDEX idCommande ON livraison (idCommande)');
        $this->addSql('CREATE INDEX idx_user_notifications ON notification (user_id, is_read)');
        $this->addSql('CREATE INDEX from_user_id ON notification (from_user_id)');
        $this->addSql('CREATE INDEX post_id ON notification (post_id)');
        $this->addSql('CREATE INDEX commentaire_id ON notification (commentaire_id)');
        $this->addSql('ALTER TABLE password_reset_tokens DROP FOREIGN KEY FK_3967A216E7927C74');
        $this->addSql('ALTER TABLE password_reset_tokens DROP FOREIGN KEY FK_3967A216E7927C74');
        $this->addSql('ALTER TABLE password_reset_tokens CHANGE date_expiration date_expiration DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL, CHANGE utilise utilise TINYINT DEFAULT 0, CHANGE email email VARCHAR(255) NOT NULL');
